#!/usr/bin/env bash
# =============================================================================
# Safety_Drone experiment runner — sweep across drone counts (process counts)
#
# Property: relative_confirmed -> (TRUE S velocity_sent)
#
# 15 explicit prove invocations (5 drone counts × 3 trace sizes), all at
# the same fixed ε. Strictly sequential. Each capped at 1000s via
# gtimeout; on timeout, the run is logged as TIMEOUT and the next
# invocation proceeds. Pressing Ctrl+C skips the current run; pressing
# it twice within 2s aborts the entire sweep.
#
# Trace files expected at:
#   traces_by_processes/trace_drone_<K>_<size>.csv
#   for K ∈ {2, 4, 6, 8, 10} and size ∈ {1k, 10k, 100k}
#
# This script does NOT generate trace files. Generate them, e.g., with:
#   for K in 2 4 6 8 10; do
#     for sz in 1000 10000 100000; do
#       lbl=$(echo $sz | sed 's/000$/k/')
#       python tools/generate_safety_drone_trace.py -n $sz -k $K -s 42 \
#         -o experiments/Safety_Drone/traces_by_processes/trace_drone_${K}_${lbl}.csv
#     done
#   done
#
# Prerequisites:
...
#   bash experiments/Safety_Drone/run_experiment_by_processes.sh           # uses default EPSILON
#   bash experiments/Safety_Drone/run_experiment_by_processes.sh 5.0       # override ε
# =============================================================================
set -uo pipefail

EXP_NAME="Safety_Drone (by drone count)"
EXP_TAG="drone"
PROP_FILE_NAME="safety_drone"
PROP_FORMULA='relative_confirmed -> (TRUE S velocity_sent)'

# Fixed ε for this entire sweep. Override via the first positional argument:
#   bash run_experiment_by_processes.sh 5.0
EPSILON="${1:-1.0}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TRACES_DIR="$SCRIPT_DIR/traces_by_processes"
LOGS_DIR="$SCRIPT_DIR/logs_by_processes"
RESULTS_FILE="$SCRIPT_DIR/results_by_processes.md"
...
for K in 2 4 6 8 10; do
    for label in 1k 10k 100k; do
        f="$TRACES_DIR/trace_drone_${K}_${label}.csv"
        [ -f "$f" ] || {
            echo "ERROR: missing trace file: $f" >&2
...
run_one() {
    ...
    local drones=$1
    local label=$2
    local trace_file="$TRACES_DIR/trace_drone_${drones}_${label}.csv"
    local run_id="${EXP_TAG}, K=${drones}, ${label}, e=${EPSILON}"
    local log_file="$LOGS_DIR/${EXP_TAG}_K${drones}_${label}_e${EPSILON}.log"
    ...
    gtime -f "%e %M %P" -o "$GTIME_TMP" \
        gtimeout --foreground "$TIMEOUT_SEC" python -m prove \
            -p "$PROP_FILE" -t "$trace_file" -e "$EPSILON" -o normal --stats \
        > "$tmp_out" 2>&1 &
    CURRENT_BG_PID=$!
    ...
    {
        echo "# Run: $run_id"
        echo "# Command: gtime python -m prove -p $PROP_FILE -t $trace_file -e $EPSILON -o normal --stats"
    ...
    printf "%s\n" "$summary"
    echo "| $EXP_TAG | $drones | $label | $EPSILON | $verdict | $nodes | $removed | $max_sum | $events | $wall_time | $max_rss_mb | $cpu_pct |" \
        >> "$RESULTS_TMP"
}
...
echo "  Fixed ε:    ${EPSILON}"
echo "  Timeout:    ${TIMEOUT_SEC}s per run"
echo "============================================"

# --- 15 explicit runs (5 drone counts × 3 trace sizes) ---------------------
...
**Sweep**: 15 explicit runs over drone counts K ∈ {2, 4, 6, 8, 10} × trace sizes {1k, 10k, 100k}, all at fixed ε = ${EPSILON}.
